// 애노테이션 유지 범위
// => CLASS 
// - .class 파일까지는 유지된다. 그러나 runtime에는 메모리에 로딩되지 않는다.
...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    // MyAnnotatin은 유지 범위가 CLASS이기 때문에 실행할 때 메모리에 로딩되지 않는다.
    // 따라서 실행 중에 애노테이션 정보를 추출할 수 없다.
    // 그러나 MyAnnotation3는 유지 범위가 RUNTIME이기 때문에 실행할 때 메모리에 로딩된다.
    // 따라서 실행 중에 애노테이션 정보를 추출할 수 있
