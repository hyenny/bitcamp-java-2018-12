...
public class Test02 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    //      단 첫 알파벳은 소문자이다.
    //      예) setNo()/getNo() => \no
    //      예) setCreateDate()/getCreateDate() => createDate
    // => 주의!
    //      필드 이름이 프로퍼티 이름이 아니다. 
    
    // 결론!
    // Board2 클래스의 프로퍼티 이름을 보면 no, title, content, createdDate, viewCount가 있다.
    // 이 프로퍼티 중에서 컬럼 이름과 같은 프로퍼티는 title, contents 이다.
    // Mybatis는 컬럼 이름과 같은 이름을 가진 프로퍼티(title, contents)에 대해서만 결과 값을 넣어 준 것이다.
    // 그래서 Board2 객체를 출력하다보면 title과 contents 값만 정상적으로 출력된다.
