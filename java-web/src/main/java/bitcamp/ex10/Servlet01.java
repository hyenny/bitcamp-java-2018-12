...
public class Servlet01 extends HttpServlet {
  ...
  protected void service(
    ...
    //    지정된 URL을 요청할 때 요청 헤더에 포함시켜 웹 서버에게 쿠키를 보낸다.
    // => HTTP 프로토콜
    //
    ...
    * HTTP/1.1 200
      ...
      Set-Cookie: name3=%ED%99%8D%EA%B8%B8%EB%8F%99 <-- URL 인코딩한 상태
      Content-Type: text/plain;charset=UTF-8
      Content-Length: 37
      Date: Wed, 03 Apr 2019 00:58:53 GMT
     */
    
    // 쿠키 생성
    // => 이름과 값으로 생성한다. 
    // => 쿠키의 유효기간을 설정하지 않으면 웹브라우저가 종료될 때까지 유지된다.
    //    웹브라우저를 종료하면 웹서버로부터 받은 유효기간이 지정되지 않은 쿠키를 모두 삭제된다.
    // => 쿠키의 사용범위를 지정하지 않으면 현재 경로에 한정한다.
    //    쿠키를 보낼 때의 URL이 /ex10/s1이라면
    //    웹브라우저는 /ex10/*
