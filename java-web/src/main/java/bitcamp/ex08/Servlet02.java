...
public class Servlet02 extends HttpServlet {
  ...
  protected void service(
    ...
    out.println("<meta http-equiv='Refresh' contents='3;url=s100'");
    out.println("/</head><body>");
    out.println("</body><html>");
    
    // 응답 헤더에 Refresh 정보를 추가한다.
    // 위에서 벌써 클라이언트에게 응답을 했는데 어떻게 응답 헤더를 출력할 수 있나요?
    // => 잊지 말자! out.prinln이 출력한 것은 출력 스트림 버퍼에 보관되어 있다.
    //    따라서 아직 클라이언트에게 응답한 상태가 아니다.
    //    그래서 다음과 같이 출력한 후에 응답 헤더 값을 추가하거나 변경할 수 있는 것이다.
    //    메서드 호출이 완료될 때 비로소 클라이언트로 응답헤더와 버퍼에 저장된 message-body가 출력된다.
    //
    // 만약 out.println()/out.printf()/out.printf() 등에서 출력한 내용이
    // 버퍼를 꽉 채웠다면 어떻게 하나요?
    // => 그러면 자동으로 클라이언트에게 응답한다.
    //    따라서 일단 클라이언트에게 응답을 하면 다음의 코드는 적용되지 않느낟. 
    //    즉 응답을 완료한 후에 헤더 값을 변경하거나 바꿀 수 없다.
    //    소용이 없다.
    
    // 다음은 버퍼를 일부로 버퍼를 채우는 코드이다.
    // 버퍼가 꽉 차면 자동으로 출력하는 것을 확인해보자.
    for (int i = 0; i < 200; i++) {
      
      out.println(i + " ====> 123456789012345678901234567890");
    }
