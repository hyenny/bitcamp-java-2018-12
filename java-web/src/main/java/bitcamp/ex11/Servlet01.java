...
public class Servlet01 extends HttpServlet {
  ...
  protected void service(
    ...
    //      그래서 서버는 클라이언트가 요청할 때마다 누구인지 알 수 없다.
    // => 이를 해결하기 위해 클라이언트가 접속하면 웹 서버는 그 클라이언트를 위한
    //      고유 번호를 발급(쿠키 이용)한다.
    //      이 고유 번호를 '세션 아이디'라 부른다.
    // => 웹 브라우저는 세션 아이디를 보관해 두었다가 그 서버에 요청할 때마다
    //      세션 아이디를 보낸다.
    //  왜? 세션 아이디는 쿠키이다.
    // => 세션 아이디 쿠키는 유효기간을 설정하지 않았기 때문에
    //      웹 브라우저는 조욜하면 세션 아이디 쿠키는 삭제된다.
    // => 세션 아이디 쿠키의 사용 범위는 웹 애플리케이션이다.
    //  예) /java-web
    //      따라서 같은 웹 애플리케이션의 서블릿을 실행할 때는 무조건 세션 아이디를 넣는다.
    //
    // 세션 아이디는 언제 발급하는가?
    // => 웹 브라우저가 세션 아이디를 보내지 안주면 발급해준다. 
