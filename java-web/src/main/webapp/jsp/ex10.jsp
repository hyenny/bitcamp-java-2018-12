...
1). request - HttpServletRequest => _jspService() 파라미터다.
2). response - HttpServletResponse => _jspService() 파라미터다.
...
6). config - ServletConfig => _jspServie()의 로컬 변수이다.
7). out - JspWriter => _jspService()의 로컬 변수이다.
8). page - 서블릿 객체를 가리킨다. 즉 this이다. => _jspService()의 로컬 변수이다.
9). exception - Throwable => _jspService()의 로컬 변수이다.
    - 이 변수는 JSP 페이지가 <%@ page errorPage="true"%>로 설정되었을 때만 존재한다.
